...
//    CGFloat width = CGRectGetWidth(self.bounds) - (allButtonWidth ? allButtonWidth : (textViewLeftMargin * 2));
    CGFloat width = CGRectGetWidth(self.bounds) - (textViewLeftMargin+allButtonWidth)*2;
    // 初始化输入框
    self.inputTextView = [[XHMessageTextView  alloc] initWithFrame:CGRectMake(allButtonWidth, kVerticalPadding, width, kInputTextViewMinHeight)];
...
//    self.recordButton.hidden = YES;
    ...
    float sendBtnX = textViewLeftMargin+allButtonWidth+width;
    
    self.sendBtn.frame = CGRectMake(sendBtnX, kVerticalPadding, 50, kInputTextViewMinHeight);
    [self.sendBtn setTitle:@"发送" forState:UIControlStateNormal];
    CALayer *layer = self.sendBtn.layer;
    [layer setCornerRadius:5];
    [layer setMasksToBounds:YES];
    [self.sendBtn addTarget:self action:@selector(sendMsgBtnClick) forControlEvents:UIControlEventTouchUpInside];
